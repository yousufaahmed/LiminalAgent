...
function WeeklySpendingGoalWithContext({ wsUrl, setGoalData: setParentGoalData, onRefresh }: { wsUrl: string, setGoalData: (data: any) => void, onRefresh?: () => React.MutableRefObject<(() => void) | null> }) {
  ...
  return (
    <div className="weekly-goal-widget">
      ...
      {/* Car-style gauge meter */}
      <div className="gauge-container">
        <svg viewBox="0 0 200 145" className="gauge-svg">
          {/* Background arc */}
          <path
          ...
          <text x="100" y="145" textAnchor="middle" fontSize="20" fontWeight="bold" fill="#1f2937">
            {Number(percentage).toFixed(0)}%
          </text>
          
          {/* 0% and 100% labels */}
          <text x="15" y="125" fontSize="12" fill="#6b7280">0%</text>
          <text x="170" y="125" fontSize="12" fill="#6b7280">100%</text>
        </svg>
      </div>
      ...
      <div className="spending-details">
        <div className="spent">
          ...
          <span className="value">{Number(spent_so_far).toFixed(2)} {currency}</span>
        </div>
        <div className="remaining">
          <span className="label">Remaining</span>
          <span className={`value ${remaining < 0 ? 'over' : ''}`}>
            {Number(remaining).toFixed(2)} {currency}
          </span>
        </div>
      </div>
...
function App() {
  ...
  return (
    <WeeklyGoalContext.Provider value={weeklyGoalData}>
      ...
      <main>
        <center><h1>UniSpend</h1></center>
        <center><h5>good kid,m.A.A.d exeter</h5></center>
      ...
      {/* Custom Image Upload for Receipts */}
      <div style={{ position: 'fixed', bottom: '20px', right: '100px', zIndex: 9998 }}>
        <input
          type="file"
          accept="image/*"
          id="receipt-upload"
